...
public class GroovyEvaluatorStackTraceTest {
  ...
  private static EvaluationObjectFactoryMock evaluationObjectFactory;
  ...
  public void arithmeticException() throws Exception {
    String code = "1/0";
    EvaluationObject seo = evaluationObjectFactory.createSeo(code);
    //when
  ...
  public void unableToResolveClass() throws Exception {
    ...
    EvaluationObject seo = evaluationObjectFactory.createSeo(code, null, commMsg(), 5);
    //when
    TryResult evaluate = groovyEvaluator.evaluate(seo, code);
    //then
    assertThat(evaluate.isError()).isTrue();
